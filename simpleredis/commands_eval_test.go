...
func TestEvalArgvAndIntegerReply(t *testing.T) {
	...
	values, err := redis.Eval(context.Background(), kongIncrbyExpireatScript, []string{"win"}, []string{"7", "1700000000"})
	if err != nil {
		t.Fatalf("Eval: %v", err)
	}
...
func TestEvalLaterSendsEvalShaNotBody(t *testing.T) {
	...
	if _, err := redis.Eval(context.Background(), kongIncrbyExpireatScript, []string{"win"}, []string{"7", "1700000000"}); err != nil {
		t.Fatalf("first Eval: %v", err)
	}
	values, err := redis.Eval(context.Background(), kongIncrbyExpireatScript, []string{"win2"}, []string{"7", "1700000000"})
	if err != nil {
...
func TestEvalTwoScriptsTwoDigests(t *testing.T) {
	...
	if _, err := redis.Eval(context.Background(), "return 1", nil, nil); err != nil {
	...
	if _, err := redis.Eval(context.Background(), "return 2", nil, nil); err != nil {
