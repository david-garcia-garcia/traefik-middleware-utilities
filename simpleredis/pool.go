...
// release peels idle-head sockets older than idleTimeout, then returns a clean conn to idleConns and frees the in-use turn, or closes it when dirty, closed, or idleConns is full at the live cap.
func (sr *SimpleRedis) release(conn *pooledConn, reusable bool) {
	...
	for len(sr.idleConns) > 0 {
		head := sr.idleConns[0]
		if now.Sub(head.lastUsed) < sr.idleTimeout {
			break
		}
		peeled = append(peeled, head)
		sr.idleConns = sr.idleConns[1:]
	}
	...
	poolClosed := sr.closed.Load()
	idleFullAtLiveCap := idleConnsFull && live >= sr.liveCap()
	if !poolClosed && !idleFullAtLiveCap {
