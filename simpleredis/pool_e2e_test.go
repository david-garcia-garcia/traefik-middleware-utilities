...
// TestLive_IdleHead proves a stale idle head is closed while the tail stays hot on dest engines.
func TestLive_IdleHead(t *testing.T) {
	runForEachLiveEngine(t, "SIMPLEREDIS_LIVE_REDIS", "SIMPLEREDIS_LIVE_DRAGONFLY", runLiveIdleHead)
}
...
func waitLiveIdleHeadClient(t *testing.T, addr string) *SimpleRedis {
	...
	for {
		if err := client.Set(context.Background(), "simpleredis-live-idle-head-probe", []byte("1"), 60); err == nil {
			return client
