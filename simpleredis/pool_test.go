...
func TestPoolWaitTimesOutWithoutExtraDial(t *testing.T) {
	...
	fake.mu.Unlock()
	redis := newTestRedis(t, Config{Host: addr, PoolSize: 2, MaxIdleConns: 2, PoolTimeout: 50 * time.Millisecond, CommandTimeout: time.Second})
...
// TestReleaseClosesWhenIdleFull proves overlapping Gets close a reusable socket when idle is already MaxIdleConns.
