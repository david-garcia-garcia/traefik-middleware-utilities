...
func (f *fakeRedis) serve(conn net.Conn) {
	...
	for {
		...
		case "MSETEX":
			...
			if f.rejectMSetEX {
				_, _ = io.WriteString(conn, "-ERR unknown command 'MSETEX'\r\n")
				break
			}
		...
		case "EVAL":
			...
			if args[1] == msetexFallbackScript {
				f.applyMSetEXEval(args)
				_, _ = io.WriteString(conn, ":1\r\n")
			} else if args[1] == kongIncrbyExpireatScript && len(args) >= 6 {
				key := args[3]
...
// applyMSetEXEval stores or deletes pairs from the fallback EVAL argv. Caller holds mu.
