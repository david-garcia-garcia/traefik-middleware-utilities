...
// seedIdle puts netConn on the idle list so the next command reuses it without dialing.
func seedIdle(client *SimpleRedis, netConn net.Conn) {
	client.mu.Lock()
	client.idle = append(client.idle, &pooledConn{
		netConn:  netConn,
...
// dialCounting seeds idle with a counting wrap of a TCP session to addr.
