...
// TestProtoPanicInDoReturnsTurnAndClosesSocket proves the deferred release in runOnConn returns the
// in-use turn AND destroys the socket when do panics, so neither the turn nor the fd leaks.
// On DestBranch the same panic loses the turn permanently (BUGS.md section 2).
func TestProtoPanicInDoReturnsTurnAndClosesSocket(t *testing.T) {
