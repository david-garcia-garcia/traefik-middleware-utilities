...
func IncrAndEval(host string) string {
	...
	values, err := client.Eval(kongIncrbyExpireatScript, simpleredis.ScriptSHA1Hex(kongIncrbyExpireatScript), []string{"yaegi-eval"}, []string{"3", "1700000000"})
	if err != nil {
...
func EvalNoScript(host string) string {
	client := simpleredis.New(simpleredis.Config{Host: host})
	values, err := client.Eval(kongIncrbyExpireatScript, simpleredis.ScriptSHA1Hex(kongIncrbyExpireatScript), []string{"yaegi-noscript"}, []string{"3", "1700000000"})
	if err != nil {
		return "eval:" + err.Error()
	}
	if len(values) != 1 || string(values[0]) != "3" {
...
func LiveVerbs(host, key string) string {
	...
	values, err := client.Eval("return 1", simpleredis.ScriptSHA1Hex("return 1"), nil, nil)
	if err != nil {
		return "eval:" + err.Error()
	}
