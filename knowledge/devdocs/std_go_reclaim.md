...
**Sleep**:
Optional `interface{ Sleep() }` on the stored value, called when the last holder's context is Done. The value stays stored and keeps its identity, and releases what is expensive to hold idle. It is not a close: the value must be resumable.
...
- Production: `reclaim.Open(ctx, key, logger, create)` (process table). Tests: `NewTable` with a short grace, or `Reset`. `logger` is required.
...
- Give the stored value `Sleep()` and `Wake()` to release and resume idle cost, and `Close()` for what only a final teardown should do. All three are optional. Under Yaegi those optional methods currently do not run (see `knowledge/debt/2026-09-11-yaegi-drops-methods-on-any.md`); compiled callers get all four events.
