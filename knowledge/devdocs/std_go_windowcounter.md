...
- Match Redis errors by `Error()` text. Exact mode (`sync_rate == 0`) returns the Redis error on that Take/Peek. Buffered mode returns a retained flush error, or probes after one missed `sync_rate`, instead of a silent nil while a local delta is pending.
