...
**SimpleRedis**:
A stdlib pooled TCP RESP client (`Init`, `Get`, `MGet`, `Set` with EX, `Del`, `Incr`, `IncrBy`, `Expire`, `ExpireAt`, `Eval`, `MSetEX`, `MSetEXAt`, `Close`). `Init` stores host, password, and database and does not dial; the first command dials.
...
- `simpleredis/yaegi_test.go` — interpreter Init/Get/Set/Del/Incr/Eval/MSetEX
