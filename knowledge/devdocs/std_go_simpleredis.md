...
- Prove with `go test ./simpleredis/...` (includes Yaegi GOPATH interp). Live pool-wait and idle-head tests skip without `SIMPLEREDIS_LIVE_REDIS` / `SIMPLEREDIS_LIVE_DRAGONFLY` or under `-short`. CI must set both. Allocation guards are `TestAlloc*` functions that call `testing.Benchmark` with `ReportAllocs` and fail on over-budget allocs/op or B/op; they do not need `-bench`. Traefik e2e is `./Test-Integration.ps1` (Redis and Dragonfly).
...
- `release` peels idle-head sockets older than `IdleTimeout` before appending or closing the returning connection. A cold head cannot sit behind a recycled tail.
