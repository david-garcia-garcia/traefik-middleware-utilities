...
- Each command is encoded with `append` + `strconv.AppendInt` into a per-connection scratch and one `net.Conn.Write` (no `bufio.Writer`). Idle scratch past 64 KiB is dropped on reusable release so a large SET cannot pin that idle conn.
