...
Import `github.com/david-garcia-garcia/traefik-middleware-utilities/simpleredis`. Callers match errors with `errors.Is` against the exported sentinels (`ErrMiss`, `ErrUnreachable`, `ErrPoolWait`, …) or `IsMiss` / `IsUnreachable` / `IsPoolWait`. The string consts (`redis:miss`, `redis:unreachable`, …) stay for display and legacy text matching. The copied sources are Apache-2.0 (`LICENSE`).
