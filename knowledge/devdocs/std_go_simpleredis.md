...
- `simpleredis/log.go` — `Msg*` constants and nil/Enabled-safe slog helpers
...
- After one complete reply, leftover unread RESP means the peer is ahead of the protocol. That command still returns its decoded value; the socket is discarded, not drained, and not pooled. Do not drain leftover bytes to resynchronise. A compliant peer leaves the reader empty, so sequential Gets still reuse one connection. Unread leftover before the next write on the same socket (AUTH then SELECT) is `redis:unreachable` and is not written. Same text as a short bulk read: the socket is unusable and the command is not. Do not tidy that to `redis:issue?` — that token is not retried. Handshake leftover still does not open a second dial (`handshakeFailed`). Those two leftover cases emit `simpleredis_socket_poisoned` and `simpleredis_auth_leftover` at Warn when a logger is set.
- Optional `Config.Logger` is frozen at `New`. Nil is silent. Debug events (`simpleredis_timeout`, `simpleredis_retry`, `simpleredis_dial`, …) are call-site `Enabled`-guarded so a nil or Warn-only logger allocates nothing on those paths. Error means an operator must act (`simpleredis_panic`, `simpleredis_noauth`, `simpleredis_not_from_new`, `simpleredis_over_free`). There is no Info tier. Never log `Pass`, Redis keys, or values.
