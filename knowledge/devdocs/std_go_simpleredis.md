...
- Each command is encoded with `append` + `strconv.AppendInt` into a per-connection scratch and one `net.Conn.Write` (no `bufio.Writer`). The allocation win is the `AppendInt`, not the single write — dest built headers as `"$" + strconv.Itoa(n) + "\r\n"`, and that concatenation was the whole cost (3 allocs to 0 on a compiled GET). `bufio` already coalesced a small command into one write, so removing it buys no syscall there; it buys the large-payload case, which took three. The price is that the payload is copied into the scratch first: measured ~1.6 µs for a 100 KiB SET against ~82 ns when `bufio` handed the slice straight to the socket. Small commands are the workload in tree, so that price is not paid. Do not justify this shape with the interpreted benches; production is compiled into Traefik and Yaegi is a portability and e2e-cost concern, not the speed target. Idle scratch past 64 KiB is dropped on reusable release so a large SET cannot pin that idle conn.
