...
- Live cap is `Config.PoolSize` (`PoolSize()`, const default 8; unused plus in-flight). `New` builds the in-use-turn channel. After `New` a `PoolSize` write does not resize. A waiter past `PoolSize` waits `PoolTimeout` (default 200ms), then `redis:unreachable`. Idle trim is `MaxIdleConns` (default 8). Pool wait is not retried. An extra return of an in-use turn when the channel is already full does not hang: it is dropped and `OverFrees()` increments. Dest used to park that goroutine forever. Correct accounting stays at `OverFrees() == 0`. A panic between `borrow` and `release` still drops the turn (`exec` does not defer `release`). The next pool-wait waiter restores missing turns when idle is empty and no still-running command holds a socket, and `LostTurns()` counts those tokens. That same waiter closes checked-out sockets whose checkout is at least the command budget `(MaxRetries+1)*(DialTimeout+IOTimeout)` old, so the abandoned fd does not stay open for the process lifetime. `AbandonedClosed()` counts those closes. A live command still within that budget is not closed. Recovery does not fire while sockets are genuinely busy. Dest used to brick the client for the process lifetime (`redis:unreachable` with a healthy Redis), then (PR #66) recover turns while leaving the fd leaked.
