...
- Each command is framed through the pooled connection's `bufio.Writer`, and every length header is `strconv.AppendInt` into a 24-byte array the connection owns (`pooledConn.lenBuf`). The allocation win is that `AppendInt`, not the number of writes: dest built headers as `"$" + strconv.Itoa(n) + "\r\n"` and that concatenation was the whole cost (compiled GET 3 allocs to 0, 100 KiB SET 5 allocs to 0). The digit scratch lives on the conn rather than as a local array because a local escapes — it is passed to `bufio.Writer.Write`, whose argument can reach the underlying `io.Writer` — which is one heap alloc per command. Dropping `bufio` for a growable per-conn scratch and one `net.Conn.Write` was implemented, measured and rejected: `bufio` already coalesces a small command into one write, so it bought no syscall on this workload (counters, TTLs, short Lua), while it cost a full payload memcpy — 100 KiB SET encode 1590 ns with the scratch against 65 ns with `bufio` — plus a 64 KiB idle-retention cap that had to be specified and tested so one large SET could not pin a parked socket. Do not justify an encoder shape with the interpreted benches: production is compiled into Traefik, and Yaegi is a portability and e2e-cost concern, not the speed target. Interpreted, the shape we ship is the more expensive one (GET encode 6330 ns / 166 allocs against 2599 ns / 69 allocs for the rejected scratch); `BenchmarkYaegiEncodeBufio` / `BenchmarkYaegiEncodeSingleWrite` keep that on the record as a cost we accept.
