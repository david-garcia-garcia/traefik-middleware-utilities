...
**SimpleRedis**:
A stdlib pooled TCP RESP client (`New(Config)`, `Get`, `MGet`, `Set` with EX, `Del`, `Incr`, `IncrBy`, `Expire`, `ExpireAt`, `Eval`, `ExecPipeline`, `Close`). `New` copies `Config` and does not dial; the first command dials. Pool, timeout, and retry knobs live on `Config` and freeze at `New`.
...
- Do not dial in Traefik `New`. Call `simpleredis.New` there; first command in `ServeHTTP` after Redis is up (`Set`, `Get`, `Incr`, `Eval`, or `ExecPipeline`).
...
- `simpleredis/commands.go` — verbs, exec, retry, ExecPipeline
- `simpleredis/resp.go` — RESP codec
- `simpleredis/yaegi_test.go` — interpreter New/Get/Set/Del/Incr/Eval/ExecPipeline
