...
- Prove with `go test ./simpleredis/...` (includes Yaegi GOPATH interp). Live pool-wait and idle-head tests skip without `SIMPLEREDIS_LIVE_REDIS` / `SIMPLEREDIS_LIVE_DRAGONFLY` or under `-short`. CI must set both. Traefik e2e is `./Test-Integration.ps1` (Redis and Dragonfly).
