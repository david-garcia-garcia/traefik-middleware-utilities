...
2. **GET previous like `windowLocked` without writing `expireAt`.** If the key is missing, GET-seed like today. If `localDelta == 0`, GET and set `redisKnown`. If `localDelta > 0`, return memory. Failed GET returns from Take. Success sets `lastRedisOK`. Alternative: treat GET miss/failure as outage — rejected; ticket forbids folding into Redis-down; Redis is up in the repro.
