...
2. **Derived overall budget `(maxRetries+1)*(DialTimeout+IOTimeout)` at `exec` entry.** Bound onto `ctx` with `context.WithDeadline` when that instant is sooner than the parent (stdlib `Dialer` / `http.Client`). `borrow`/`dial`/`do` take only `ctx`. Per-command `SetDeadline` is still `min(IOTimeout, remaining)` because `net.Conn` has no context. No `Config.CommandTimeout`. Alternative: thread a parallel `time.Time` next to `ctx` — rejected; that duplicated stop checks and raced `select` with `ctx.Done()`. Alternative: new knob — rejected; operators already have Dial/IO/retries; context is the caller-side cap.

3. **`exec(ctx, args)` and every public verb takes `ctx` first.** Backoff is `select` on timer and `ctx.Done()`, not `time.Sleep`. Cancel watcher closes `netConn` on `ctx.Done()`. Alternative: `*Context` twins with unadorned `Background()` wrappers — rejected; that lets request-path callers skip cancel.

4. **Error tokens.** `ctx.Err()` for cancel / caller deadline. Library-owned `DeadlineExceeded` maps to `redis:timeout` at `exec` (`http.Client` `didTimeout` shape). `shouldRetry` retries neither. Alternative: collapse cancel to `redis:unreachable` — rejected; that hides which failure it was.
