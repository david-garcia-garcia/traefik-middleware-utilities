...
- Nil `create` and create panic share the existing create-error branch via `endBusySlot`. Wrap panic as `fmt.Errorf("reclaim: create %q: panic: %v", key, recovered)`. Nil create: `fmt.Errorf("reclaim: create %q: nil create", key)`.
