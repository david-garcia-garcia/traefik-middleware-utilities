...
contexts are Done and its incarnation has ended. A watcher started for a holder whose `Done` is
nil SHALL exit without dropping that holder when the incarnation has ended, even if `ctx.Err()`
is still unset. When that holder's `Err()` is set while the incarnation is still live, the table
SHALL still drop the holder.
