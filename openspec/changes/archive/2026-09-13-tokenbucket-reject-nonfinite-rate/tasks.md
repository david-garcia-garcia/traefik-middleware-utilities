...
- [x] 1.2 Add `tokenbucket/repro_inf_rate_test.go` (`TestRepro_InfRateRejected`) asserting New rejects +Inf (and `-Inf`) as `errRate`. After the fix New MUST error; do not assert second Allow fail-open as the post-fix contract.
