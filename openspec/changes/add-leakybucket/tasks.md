...
- [x] 1.3 Unit tests: pour to cap then deny; idle drain; Level after Δt; until-not-full; invalid New; no HTTP/source reads
...
- [x] 2.3 Fake TCP unit tests: Eval encoding; two instances both pours count (no last-write-wins); over-allow bounded by sync interval
- [x] 2.4 Memory vs Redis agreement on the same leak/capacity sequence when `sync_rate=0`
