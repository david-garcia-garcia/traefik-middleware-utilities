...
- [x] 1.1 Add `ratelimit/` with `New(redis *simpleredis.SimpleRedis, syncRate time.Duration)`, `Take`/`Allow`, opaque key + limit + whole-second window, sliding estimate, increment-then-compare
...
- [x] 3.1 Live tests table-driven on `RATELIMIT_LIVE_REDIS` / `RATELIMIT_LIVE_DRAGONFLY`; skip on `testing.Short` or missing addrs; exact, two-client share, sliding boundary on both engines
- [x] 3.2 Yaegi live: GOPATH copy of non-test `ratelimit` and `simpleredis` sources; stdlib only; `useunsafe` false; compiled test owns start/skip; probe calls Take
- [x] 3.3 CI `test` job: Redis 7 alpine + Dragonfly `v1.40.2` service containers, both env vars set, no `-short`
- [x] 3.4 Run `go test ./ratelimit/...` on this host until passing (short/unit always; live when engines are up)

## 4. Docs and specs

- [x] 4.1 README Libraries/Layout/Tests: this primitive instead of leaky `bucket/`
- [x] 4.2 Usage packet `knowledge/devdocs/std_go_ratelimit.md` and `index_std_go.md` row
- [x] 4.3 Confirm change specs `std_go_ratelimit_sliding-take` and `std_go_ratelimit_sync-flush` match the API
