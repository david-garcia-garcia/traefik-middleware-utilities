...
- Add package `ratelimit/` (`package ratelimit`): caller injects `*simpleredis.SimpleRedis`; `New(redis, syncRate)`; `Take`/`Allow` on an opaque key + limit + window; sliding estimate only.
...
- Usage packet `knowledge/devdocs/std_go_ratelimit.md`.

## Capabilities

### New Capabilities

- `std_go_ratelimit_sliding-take`: Sliding-window `Take`/`Allow` — opaque key (caller owns identity), integer Redis counters for current and previous windows, estimate formula, error propagation, unit/Yaegi/live proof of admit/deny and boundary (no fixed-window double).
- `std_go_ratelimit_sync-flush`: `sync_rate` exact vs buffered flush — `Incr`+`Expire` vs EVAL INCRBY+EXPIREAT-if-new, two-client share without last-write-wins, Sleep/Wake/Close ticker lifecycle, CI starts Redis and Dragonfly.
...
- New `ratelimit/` (stdlib + this module’s `simpleredis` only).
- README Libraries/Layout/Tests. `.github/workflows/ci.yml` `test` job service containers for Redis and Dragonfly plus `RATELIMIT_LIVE_*` env.
- `knowledge/devdocs/std_go_ratelimit.md` and `index_std_go.md` row.
