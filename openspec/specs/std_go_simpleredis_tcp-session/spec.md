...
The close decision SHALL stay: close when the client is closed, or when the idle list is already at `MaxIdleConns`. Otherwise park. `lastUsed` SHALL still be stamped on the reusable path before the socket is parked, including when the later verdict is close.
