...
- **THEN** the line is `do: redis error` at Warn
...
- **THEN** the line is `exec: canceled` or `exec: deadline exceeded` at Debug
...
### Requirement: A peer reply is not published
A log line MUST NOT contain any byte of a Redis error reply. When the failure is not a sentinel this package owns (`redis:unreachable`, `redis:timeout`, `redis:noauth`), the message SHALL be `<site>: redis error`. The session MUST NOT parse, trim, or republish peer text. The caller SHALL still receive the peer's full text.

Two replies are why: Redis 7.4 answers `AUTH` against a nopass default user with `ERR AUTH <password> called without any password configured for the default user`, which is not an AUTH-class prefix and so is never mapped to `redis:noauth`; and an unknown verb is refused with the command's own arguments quoted back (`ERR unknown command 'MSETEX', with args beginning with: '2', '<key>', '<value>'`), which is Redis key names and values.

#### Scenario: An AUTH reply that echoes the password stays off the line
- **WHEN** the peer answers AUTH with `ERR AUTH <password> called without any password configured for the default user`
- **THEN** the handshake line is `dial: redis error` at Warn with `verb` `AUTH`
...
- **THEN** the line is `do: redis error` at Debug
- **AND** the captured output does not contain the key name

#### Scenario: A server state reply is not published either
- **WHEN** AUTH is answered with `LOADING Redis is loading the dataset in memory`
- **THEN** the lines are `dial: redis error` at Warn and `do: redis error` at Debug
...
- **AND** a dropped in-use-turn return emits `simpleredis_over_free` at Error, and leftover after a complete reply emits `simpleredis_socket_poisoned` at Warn

#### Scenario: A panic is not logged here
- **WHEN** `do` panics inside `runOnConn`
- **THEN** the session MUST NOT recover, log, and re-raise: Traefik already logs panics
- **AND** the panic still reaches the caller
- **AND** the deferred release still runs, the in-use turn is returned, the socket is closed, and `OverFrees()` is `0`
...
Interpreted code SHALL construct a client with a `*slog.Logger` on `Config`, observe `simpleredis_open`, and then fail one command so that a site line is emitted too. The site emit SHALL be `Logger.Debug` (the same slog surface the named events use). Call sites MUST NOT nil-check the logger: `New` assigns a discard logger when `Config.Logger` is nil. Those tests MUST use GOPATH with stdlib symbols only. Those tests MUST NOT start Traefik.
