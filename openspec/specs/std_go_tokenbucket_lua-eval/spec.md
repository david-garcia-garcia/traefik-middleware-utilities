...
Live tests SHALL call Allow against Redis and Dragonfly using `TOKENBUCKET_LIVE_REDIS` and `TOKENBUCKET_LIVE_DRAGONFLY`. Tests SHALL skip when those addrs are unset or under `-short`. CI SHALL start both engines, set both variables, and MUST NOT skip. Yaegi live SHALL run the same scenarios; the compiled test owns start/skip; the interpreted probe calls Allow.
