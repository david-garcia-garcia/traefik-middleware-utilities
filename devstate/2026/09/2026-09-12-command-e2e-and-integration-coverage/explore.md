...
- **Traefik: path-dispatched micro suite, not one request with every header.** Human asked at `ServeHTTP`. `ServeHTTP` switches on the path case and runs that verb (or recover / hold / drop). Pester gets one `It` per case per engine (`GET /redis/get`, `GET /dragonfly/msetexat`, …). Spec `std_go_simpleredis_resp-commands` “same request SHALL also call … every verb” is reshaped to one request per case. Exact `/redis` and `/dragonfly` stay health Set+Get. No new compose routers. MSetEXAt is `/redis/msetexat` + `/dragonfly/msetexat`, not a missing header on the dump. Do not expand recover or drop-relay to the full verb set (`requirement.md` Out of scope) — they become `/redis/recover` and `/redis/drop` (and dragonfly twins) with the same Set+Get / Incr+Eval jobs they have today. `?hold=` can move to `/redis/hold`.
...
- Q: Expand `?recover=1` or drop-relay to the full verb set?
  ...
  Decision: resolved — do not. They become `/redis/recover` and `/redis/drop` (and dragonfly twins) with the same Set+Get / Incr+Eval jobs.
  By: explore
...
- Q: Replace the Traefik one-request-all-headers dump with a path-dispatched micro suite (middleware switches on path; one Pester `It` per case)?
  Rank: bounded asked — human asked at `e2e/simpleredisprobe/plugin.go:95`; 5 call-site groups enumerated and migratable here: `plugin.go` ServeHTTP, `scripts/integration-tests.Tests.ps1`, `Test-Integration.ps1` health URLs, `docker-compose.yml` PathPrefix (already matches `/redis/<case>`; no new routers), `openspec/specs/std_go_simpleredis_resp-commands/spec.md` Traefik requirement and scenarios
  Decision: resolved — take path cases. Last segment is the case. Exact `/redis` `/dragonfly` remain health. Handshake routers stay Set+Get 502.
  By: explore
