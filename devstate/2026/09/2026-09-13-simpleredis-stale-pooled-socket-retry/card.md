...
**Developers.** After an I/O failure on a socket taken from idle, later attempts of that SimpleRedis command skip idle and dial instead of popping the next dead unused socket. `borrowSocket` is the only borrow path. Sequential Gets after a full idle vintage drop succeed at default MaxRetries. Lost-reply Incr with MaxRetries off still fails with `redis:unreachable`. The SimpleRedis usage packet and `std_go_simpleredis_tcp-session` name that sequential recovery.
...
## Specs
- [std_go_simpleredis_tcp-session](https://github.com/david-garcia-garcia/traefik-middleware-utilities/blob/2026-09-13-simpleredis-stale-pooled-socket-retry/openspec/changes/archive/2026-09-13-simpleredis-stale-pooled-socket-retry/proposal.md) — modified
