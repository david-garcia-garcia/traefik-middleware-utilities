...
- Q: What chaos duration and lifecycle iteration counts keep the default suite in the low seconds and keep `go test -race` from timing out?
  ...
  Decision: assumed — `testing.Short()` skips chaos and lifecycle. Default: 24 goroutines × 1s chaos (PoolSize 4, keys k0..k63); lifecycle 200 New/use/Close cycles. When `raceDetectorOn`: chaos 8 goroutines × 250ms; lifecycle 40 cycles; concurrent MSetEX 4×8 (stays on; Unit race PASS on 34750577659). Skip only `TestYaegiErrorpath_PoolWait` under `-race` (stacks in `yaegi/interp`, not this client). Quiesce before at-rest asserts. No peak-socket assertion.
  By: implement
