...
| SimpleRedis | Current | `simpleredis/` | Stdlib RESP client (GET/MGET/SET/DEL/INCR/EXPIRE/EVAL/MSetEX/ExecPipeline). Middlewares construct this with `New(Config)` instead of inventing a Redis client. Apache-2.0 (copied from crowdsec-bouncer). |
...
**SimpleRedis** (`simpleredis/`) is the shared Redis/Dragonfly session. `New(Config)` stores host, password, and database; the first command dials. Use it for GET/SET/INCR/EVAL/MSetEX and mixed-verb `ExecPipeline`. It does not implement a rate-limit algorithm.
