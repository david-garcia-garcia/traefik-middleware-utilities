...
// Digests of the reused probe scripts, computed once at package init.
var (
	kongIncrbyExpireatDigest = simpleredis.ScriptSHA1Hex(kongIncrbyExpireatScript)
	timeWaitHoldDigest       = simpleredis.ScriptSHA1Hex(timeWaitHoldScript)
	ttlScriptDigest          = simpleredis.ScriptSHA1Hex(ttlScript)
)
...
func (m *middleware) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if hold := req.URL.Query().Get("hold"); hold != "" {
		if _, err := m.client.Eval(ctx, timeWaitHoldScript, timeWaitHoldDigest, nil, []string{hold}); err != nil {
	...
	evalValues, err := m.client.Eval(ctx, kongIncrbyExpireatScript, kongIncrbyExpireatDigest, []string{evalKey}, []string{"3", expireUnix})
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadGateway)
		return
	}
	...
	evalAgainValues, err := m.client.Eval(ctx, kongIncrbyExpireatScript, kongIncrbyExpireatDigest, []string{evalAgainKey}, []string{"3", expireUnix})
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadGateway)
		return
	}
	...
	rw.Header().Set("X-SimpleRedis-EvalDigest", kongIncrbyExpireatDigest)
	...
	ttlValues, err := m.client.Eval(ctx, ttlScript, ttlScriptDigest, []string{msetexKey}, nil)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadGateway)
		return
	}
...
func (m *middleware) writeDropHeaders(ctx context.Context, rw http.ResponseWriter, prefix string) {
	...
	evalValues, evalErr := m.dropClient.Eval(ctx, kongIncrbyExpireatScript, kongIncrbyExpireatDigest, []string{dropEvalKey}, []string{"3", strconv.FormatInt(time.Now().Add(60*time.Second).Unix(), 10)})
	evalText := "ok"
	if len(evalValues) == 1 {
